...
function validateEnter(value: unknown, name: string): void {
  ...
  if (enter.stagger !== undefined) number(enter, "stagger", name, { integer: true, min: 0, max: 8 });
}

function validateMotion(value: unknown, name: string, primitive: string): void {
  ...
  if (motion.type === "spin") {
    ...
    if (!["cw", "ccw"].includes(String(motion.direction))) throw usageError(`${name}.direction is invalid.`);
    if (!["slow", "medium", "fast"].includes(String(motion.speed))) throw usageError(`${name}.speed is invalid.`);
    return;
  }
  if (motion.type === "path") {
  ...
  if (motion.type === "drift") {
    ...
    if (!["in", "out"].includes(String(motion.zoom))) throw usageError(`${name}.zoom is invalid.`);
    if (!["left", "right", "up", "down", "none"].includes(String(motion.direction))) throw usageError(`${name}.direction is invalid.`);
    if (!["slow", "medium", "fast"].includes(String(motion.speed))) throw usageError(`${name}.speed is invalid.`);
    return;
  }
  throw usageError(`${name}.type is invalid.`);
...
function validateSelector(
  ...
  mediaPrimitives: ReadonlyMap<string, BundleMediaPrimitive>,
...
function validateAudio(
  ...
  if (audio.volume !== undefined) number(audio, "volume", name, { min: 0, max: 1 });
  ...
  for (const [index, trackValue] of audio.tracks.entries()) {
    ...
    if (trackIds.has(id)) throw usageError(`Playlist audio track id is duplicated: ${id}.`);
    trackIds.add(id);
    const mediaId = string(track, "media_id", trackName, 256, MEDIA_ID_PATTERN);
    const actual = mediaPrimitives.get(mediaId);
    if (!actual) throw usageError(`${trackName} references media missing from the bundle: ${mediaId}.`);
    if (actual !== "audio") throw usageError(`${trackName} references ${actual} media; soundtrack tracks must be audio: ${mediaId}.`);
    referenced.add(mediaId);
  }
  return trackIds;
}

function validateAudioCue(value: unknown, name: string, trackIds: ReadonlySet<string> | undefined): void {
  ...
  if (!trackIds) throw usageError(`${name} requires a playlist audio soundtrack.`);
  if (!trackIds.has(track)) throw usageError(`${name}.track must name a track id in audio.tracks.`);
}
...
export function validatePlaylistWrite(
  ...
  for (const [pageIndex, pageValue] of playlist.pages.entries()) {
    ...
    if (page.audio_cue !== undefined) validateAudioCue(page.audio_cue, `${pageName}.audio_cue`, trackIds);
    ...
    for (const [primitiveIndex, primitiveValue] of page.primitives.entries()) {
      ...
      if (primitive.primitive === "iframe") {
        ...
        if (primitive.motion !== undefined) validateMotion(primitive.motion, `${primitiveName}.motion`, "iframe");
        iframeCount += 1;
        continue;
      }
      ...
      exact(
        ...
        category === "image"
          ...
          : ["id", "primitive", "selector", "muted", "loop", "rect", "layer", "content_fit", "enter", "motion"],
        primitiveName,
      );
      if (primitive.motion !== undefined) validateMotion(primitive.motion, `${primitiveName}.motion`, category);
